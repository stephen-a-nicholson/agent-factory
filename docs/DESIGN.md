...
Three rules diverge slightly from the table above, all because of decisions already made and documented earlier in this file. `required_tags` and `endpoint_scale_to_zero` both say "endpoint" because the original rule table was written before phase 2 established that this project has no bundle-declared `model_serving_endpoints` resource at all (see section 4): `required_tags` only checks `jobs`, the one resource type here that actually carries `tags`, and `endpoint_scale_to_zero` checks `model_serving_endpoints` for forward compatibility (a future domain, or a different project reusing this gates package, might declare one) but always finds nothing to check in this repository today. `naming_convention` drops the "endpoint names are kebab-case" clause for the same reason: the one kebab-case name in this project, `<domain>-agent-<target>` (`build_endpoint_name` in `src/agent/deploy.py`), belongs to that same unmanaged serving endpoint, so there is no bundle resource field to check it against; the vector search endpoint, the one endpoint-shaped resource that does exist in the bundle, is correctly snake_case like every other resource key here, not kebab-case. `job_has_test` (warn) accepts either `tests/test_<module>.py` or `tests/test_<parent_package>_<module>.py` for a task's entry point, since this project's own test files are not consistently named one way (compare `src/ingest/documents.py` → `tests/test_documents.py` with `src/evaluate/rag.py` → `tests/test_evaluate_rag.py`), and warn severity means it is a nudge, not a gate that should force a rename to satisfy a stricter pattern.
...
The repo doubles as a custom bundle template: `databricks bundle init https://github.com/<user>/agent-factory` asks for a project name and a domain name and produces a ready-to-run bundle with an empty domain folder and the F1 domain as an example. `databricks_template_schema.json` lives at the repo root; the actual scaffolded content is `template/{{.project_name}}/`, generated by `factory/build_template.py` (`uv run python -m factory.build_template`) rather than hand-maintained, since `databricks bundle init` needs a real `template/` subdirectory (`template_dir: "."`, pointing it at the repo root directly, fails outright — confirmed against a real deploy) and cannot symlink whole directories into it (also confirmed against a real deploy: only file symlinks work, a directory symlink fails the copy). `build_template.py` walks every shared directory (`factory/`, `src/`, `gates/`, `tests/`, `.github/`, `resources/core/`, `domains/f1/`, plus a handful of top-level files) and creates one file symlink per file, so the template and the reference implementation cannot drift the way DESIGN.md always intended, just achieved by symlinking individual files rather than by literally sharing one directory tree. Two pieces of real (non-symlinked) content exist only under `template/`: a commented `domain.yml.tmpl` skeleton for the user's own domain, and a fresh `docs/PLAN.md.tmpl` (this repo's own `docs/PLAN.md` is a build journal specific to this repo, deliberately excluded from a fresh scaffold). `databricks.yml`'s bundle name is a mechanical text substitution done by `build_template.py` at build time, not a hand-duplicated file, for the same reason. See `docs/PLAN.md` phase 7 notes for the full detail on what actually broke while getting this working, including the one genuinely surprising finding: only a file whose name ends in `.tmpl` gets its content rendered by `bundle init` at all (and the suffix stripped); every other file, symlinked or not, is copied byte for byte.
